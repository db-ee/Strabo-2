...
import it.unibz.krdb.obda.strabon.loader.TripleTableLoader;
import it.unibz.krdb.obda.strabon.loader.VerticalPartitioningLoader;
import it.unibz.krdb.obda.strabon.utils.Utils;
...
public class DataLoader {
    ...
    public static String appName = "Strabo2 Data Loader";
    ...
    private static String hiveTableFormat = "Parquet";   //  TextFile, SequenceFile, RCfile, ORC, and Parquet
    private static boolean createSeparateGeometryTable = true;
    private static String inputFormat = "Text";

    public static void main(final String[] args) throws Exception {
        final InputStream inStream = DataLoader.class.getClassLoader().getResourceAsStream(loj4jFileName);
        ...
        final Option geomTableOpt
                = new Option("geom", "createGeomTable", true, "To create separate geometry table");
        geomTableOpt.setRequired(false);
        options.addOption(geomTableOpt);
        ...
        if (cmd.hasOption("createGeomTable")) {
            final String geomTableValue = cmd.getOptionValue("createGeomTable");
            if (geomTableValue.compareTo("false") == 0) {
                createSeparateGeometryTable = false;
                logger.info("Not creating separate table for geometries");
            }
        ...
        if (generateVP) {
            ...
            final VerticalPartitioningLoader vp_loader
                    ...
                    hiveTableFormat, createSeparateGeometryTable, inputFormat);
            vp_loader.load();
            executionTime = System.currentTimeMillis() - startTime;
