package it.unibz.krdb.obda.strabon.utils;

import java.io.Serializable;
import java.util.List;

import it.unibz.krdb.obda.strabon.DataLoader;
...
public class NamespaceDictionary implements Serializable {
    ...
    // ----- STATIC MEMBERS -----
    protected static final Logger logger = Logger.getLogger(DataLoader.appName);
...
//            spark.sql(String.format("INSERT INTO %1$s SELECT * FROM tmp_nsprefixes", nsPrefTableName));
            spark.sql(String.format("DROP TABLE IF EXISTS %s", nsPrefTableName));
