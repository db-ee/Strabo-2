...
public class QueryExecutor {
	...
	// cacheSpatialIndex will create an in-memory spatial index during
	// initialization
	...
	public static void main(String[] args) throws Exception {
		{
			...
			try {
				...
				Map<String, String> predDictionary;
				
				if (spark.catalog().tableExists(database, "dict")) {
					// read prefixes from table
					List<Row> dataRows = spark.sql("Select * from dict").collectAsList();
					predDictionary = new HashMap<String, String>();
					for (Row row : dataRows) {
						predDictionary.put(row.getString(2), row.getString(4));
					}
					
				}
				else {
					predDictionary = readPredicatesFromHadoop(propDictionary, fs);
				}
				...
				// key is the prefix
				if (spark.catalog().tableExists(database, "nsprefixes1")) {
					// read prefixes from table
				...
				for (GeometryTable asWKTsubprop : geometryTables) {
					
					String tblName = asWKTsubprop.getTablename();
					...
					if (cacheSpatialIndex) {
						...
						log.debug("Caching index for geometry table " + tblName + "(" + asWKTsubprop + ") finished in "
				...
				for (String sparql : sparqlQueries) {
					try {
						...
						for (int k = 0; k < tempTables.size(); k++) {
							try {
								spark.sql("DROP VIEW " + StrabonParameters.TEMPORARY_SCHEMA_NAME + "."
										+ tempTables.get(k));
							} catch (Exception ex) {
						...
						for (int k = 0; k < sql.getTempQueries().size(); k++) {
							...
							if (tempDataset.isEmpty()) {
								...
								// return empty result
								log.debug("Execution finished in " + (System.currentTimeMillis() - start)
										+ " with 0 results.");
								exec += "Execution finished in " + (System.currentTimeMillis() - start)
										+ " with 0 results.\n";
								emptyResult = true;
								break;
							}
						...
						if (sql.getSpatialTable() != null) {
							...
							log.debug("spatial table to remove: "
									+ sql.getSpatialTableToRemove().getFunctionSymbol().getName());
							JavaRDD rdd = RangeQuery.SpatialRangeQuery(
									cachedIndexes.get(sql.getSpatialTableToRemove().getFunctionSymbol().getName()
											.replace(StrabonParameters.TEMPORARY_SCHEMA_NAME + ".", "")),
									g, sql.isCondsiderSpatialBoundary(), true);
							// JavaRDD rdd =
							// RangeQuery.SpatialRangeQuery(cachedIndexes.get(sql.getSpatialTableToRemove().getFunctionSymbol().getName().replace(StrabonParameters.TEMPORARY_SCHEMA_NAME+".","")),
							// g, true, true);
							// gf.createGeometry(g);
							...
							// Dataset<Row> filter = spark.sql("Select geometry as o, _c1 as s from
							// "+StrabonParameters.TEMPORARY_SCHEMA_NAME + "." +temp);
						...
						if (sql.getSpatialTable() != null) {
							sqlString = sqlString.replace(sql.getSpatialTableToRemove().getFunctionSymbol().getName(),
									StrabonParameters.TEMPORARY_SCHEMA_NAME + "." + sql.getSpatialTable());
	...
	private static void save_exec(String executionFinished, FileSystem fs)
