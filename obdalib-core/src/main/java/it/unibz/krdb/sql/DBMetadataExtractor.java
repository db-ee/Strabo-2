 ...
 * 
 * HSQLDB treats unquoted identifiers as case insensitive in SQL but stores them
 * in upper case; it treats quoted identifiers as case sensitive and stores them
 * verbatim. All jdbcDatabaseMetaData methods perform case-sensitive comparison
 * between name (pattern) arguments and the corresponding identifier values as
 * they are stored in the database.
 *
 * HSQLDB uses the standard SQL identifier quote character (the double quote
 * character); getIdentifierQuoteString() always returns ".
 * 
 * 
 * 
 * PostgreSQL ----------
 ...
 * 
 * Quoted identifier is formed by enclosing an arbitrary sequence of characters
 * in double-quotes ("). (To include a double quote, write two double quotes.)
 * 
 ...
 * without any spaces in between, for example U&"foo".
 * 
 * 
 * H2 --
 * 
 ...
 * 
 * Names are not case sensitive (but it appears that the upper-case is the
 * canonical form).
 * 
 * Quoted names are case sensitive, and can contain spaces. Two double quotes
 * can be used to create a single double quote inside an identifier.
 * 
 * 
 * MS SQL Server -------------
 * 
...
public class DBMetadataExtractor {
	...
	public static DBMetadata createDummyMetadata(String driver_class) {
		return new DBMetadata(driver_class, null, null, "", new QuotedIDFactoryLowerCase());
	}
	...
	public static DBMetadata createMetadata(Connection conn) throws SQLException {
		...
		DBMetadata metadata = new DBMetadata(md.getDriverName(), md.getDriverVersion(), productName,
				md.getDatabaseProductVersion(), idfac);
	 ...
	 * Retrieves the database metadata (table schema and database constraints)
	 * 
	 * This method either uses the given list of tables or if it is null then it
	 * retrieves all the complete list of tables from the connection metadata
	 * 
	 * @return The database metadata object.
	 */

	public static void loadMetadata(DBMetadata metadata, Connection conn, Set<RelationID> realTables)
			throws SQLException {
		...
		if (productName.contains("Oracle")) {
			...
			if (realTables == null || realTables.isEmpty()) {
				if (productName.contains("DB2"))
					...
					seedRelationIds = getTableListDefault(md, ImmutableSet.of("sys", "INFORMATION_SCHEMA"));
				else
	...
	public static void createStrabonMetadata(DBMetadata metadata, Set<RelationID> realTables) throws SQLException {
		...
		for (RelationID seedId : seedRelationIds) {
			...
			RelationID relationId = RelationID.createRelationIdFromDatabaseRecord(seedId.getSchemaName(),
					seedId.getTableName());
			currentRelation = metadata.createDatabaseRelation(relationId);
			QuotedID attributeId = QuotedID.createIdFromDatabaseRecord("s");
			if (printouts)
				System.out.println("         " + relationId + "." + attributeId);
			...
			QuotedID attributeId2 = QuotedID.createIdFromDatabaseRecord("o");
			if (printouts)
	...
	private static List<RelationID> getTableList(String defaultTableSchema, Set<RelationID> realTables,
			QuotedIDFactory idfac) throws SQLException {

		List<RelationID> fks = new LinkedList<>();
		for (RelationID table : realTables) {
			// defaultTableSchema is non-empty only for Oracle and DUAL is a special Oracle
			// table
			if (table.hasSchema() || (defaultTableSchema == null) || table.getTableName().equals("DUAL"))
				fks.add(table);
			else {
				RelationID qualifiedTableId = idfac.createRelationID(defaultTableSchema,
	...
	/**
	 * Retrieve the table and view list from the JDBC driver (works for most
	 * database engines, e.g., MySQL and PostgreSQL)
	 */
	private static List<RelationID> getTableListDefault(DatabaseMetaData md, ImmutableSet<String> ignoredSchemas)
			throws SQLException {
	 ...
	 */
	private static List<RelationID> getTableList(Connection conn, RelationListProvider relationListProvider,
			QuotedIDFactory idfac) throws SQLException {
	...
	private static final class OracleRelationListProvider implements RelationListProvider {
		...
		public String getQuery() {
			...
			return "SELECT table_name as object_name FROM user_tables WHERE " + "   NOT table_name LIKE 'MVIEW$_%' AND "
					+ "   NOT table_name LIKE 'LOGMNR_%' AND " + "   NOT table_name LIKE 'AQ$_%' AND "
					+ "   NOT table_name LIKE 'DEF$_%' AND " + "   NOT table_name LIKE 'REPCAT$_%' AND "
					+ "   NOT table_name LIKE 'LOGSTDBY$%' AND " + "   NOT table_name LIKE 'OL$%' " + "UNION ALL "
					+ "SELECT view_name as object_name FROM user_views WHERE " + "   NOT view_name LIKE 'MVIEW_%' AND "
					+ "   NOT view_name LIKE 'LOGMNR_%' AND " + "   NOT view_name LIKE 'AQ$_%'";
		}
	...
	/**
	 * Table list for DB2 database engine (not needed now -- use JDBC metadata
	 * instead)
	 */
	/*
	 * private static final RelationListProvider DB2RelationListProvider = new
	 * RelationListProvider() {
	 * 
	 * @Override public String getQuery() { return "SELECT TABSCHEMA, TABNAME " +
	 * "FROM SYSCAT.TABLES " + "WHERE OWNERTYPE='U' AND (TYPE='T' OR TYPE='V') " +
	 * "     AND TBSPACEID IN (SELECT TBSPACEID FROM SYSCAT.TABLESPACES WHERE TBSPACE LIKE 'USERSPACE%')"
	 * ; }
	 * 
	 * @Override public RelationID getTableID(ResultSet rs) throws SQLException {
	 * return
	 * RelationID.createRelationIdFromDatabaseRecord(rs.getString("TABSCHEMA"),
	 * rs.getString("TABNAME")); } };
	 */

	/**
	 * Table list for MS SQL Server database engine (not needed now -- use JDBC
	 * metadata instead)
	 */
	/*
	 * private static final RelationListProvider MSSQLServerRelationListProvider =
	 * new RelationListProvider() {
	 * 
	 ...
	 * 
	 * @Override public RelationID getTableID(ResultSet rs) throws SQLException {
	 * //String tblCatalog = rs.getString("TABLE_CATALOG"); return
	 * RelationID.createRelationIdFromDatabaseRecord(rs.getString("TABLE_SCHEMA"),
	 * rs.getString("TABLE_NAME")); } };
	 */
	...
	private static final DatatypeNormalizer OracleTypeFixer = new DatatypeNormalizer() {
		...
		public int getCorrectedDatatype(int dataType, String typeName) {
			...
			// TODO
			// Oracle bug here - wrong automatic typing - Date vs DATETIME - driver
			// ojdbc16-11.2.0.3
	 ...
	 * Connection connection, ResultSet rsColumns, String tableSchema, String
	 * tableName) throws SQLException {
	 * 
	 * log.debug("=============== COLUMN METADATA ========================");
	 * 
	 * if (dbMetadata.getDatabaseProductName().contains("DB2")) { // Alternative
	 * solution for DB2 to print column names // Queries directly the system table
	 * SysCat.Columns // ROMAN (20 Sep 2015): use PreparedStatement instead? try
	 * (Statement st = connection.createStatement()) { String sqlQuery =
	 * String.format("SELECT colname, typename \n FROM SysCat.Columns \n" +
	 * "WHERE tabname = '%s' AND tabschema = '%s'", tableName, tableSchema);
	 * 
	 * try (ResultSet results = st.executeQuery(sqlQuery)) { while (results.next())
	 * { log.debug("Column={} Type={}", results.getString("colname"),
	 * results.getString("typename")); } } } } else { // Generic procedure based on
	 * JDBC ResultSetMetaData columnMetadata = rsColumns.getMetaData(); int count =
	 * columnMetadata.getColumnCount(); for (int j = 0; j < count; j++) { String
	 * columnName = columnMetadata.getColumnName(j + 1); String value =
	 * rsColumns.getString(columnName); log.debug("Column={} Type={}", columnName,
	 * value); } } }
	 */
	...
	private static void getPrimaryKey(DatabaseMetaData md, DatabaseRelationDefinition relation) throws SQLException {
		...
		try (ResultSet rs = md.getPrimaryKeys(null, id.getSchemaName(), id.getTableName())) {
			...
			while (rs.next()) {
				// TABLE_CAT is ignored for now; assume here that relation has a fully specified
				// name
				RelationID id2 = RelationID.createRelationIdFromDatabaseRecord(rs.getString("TABLE_SCHEM"),
						rs.getString("TABLE_NAME"));
				if (id2.equals(id)) {
	 ...
	 * @throws SQLException
	 */
	private static void getUniqueAttributes(DatabaseMetaData md, DatabaseRelationDefinition relation)
			throws SQLException {
		...
		try (ResultSet rs = md.getIndexInfo(null, id.getSchemaName(), id.getTableName(), true, true)) {
			...
			String currentName = null;
			while (rs.next()) {
				// TYPE: tableIndexStatistic - this identifies table statistics that are
				...
				if (builder != null) {
					...
					// tableIndexStatistic
					// CARDINALITY int => When TYPE is tableIndexStatistic, then this is the number
	 ...
	 * 
	 */
	private static void getForeignKeys(DatabaseMetaData md, DatabaseRelationDefinition relation, DBMetadata metadata)
			throws SQLException {
		...
		try (ResultSet rs = md.getImportedKeys(null, relationId.getSchemaName(), relationId.getTableName())) {
			...
			String currentName = null;
			while (rs.next()) {
				RelationID refId = RelationID.createRelationIdFromDatabaseRecord(rs.getString("PKTABLE_SCHEM"),
