...
public class StrabonStatement implements OBDAStatement {
			...
			for (CQIE cq : program.getRules()) {
				...
				if (head.getName().startsWith(OBDAVocabulary.TEMP_VIEW_QUERY)) {
					...
					for (String t : cq.getSignature()) {
						try {
							if (isGeometry(t, cq)) {
								replacement.addAttribute(questInstance.getMetaData().getQuotedIDFactory()
										.createAttributeID(t.toString()), Types.BINARY, "geometry", false);
							} else {
								replacement.addAttribute(questInstance.getMetaData().getQuotedIDFactory()
										.createAttributeID(t.toString()), Types.VARCHAR, "VARCHAR", false);
							}
			...
			try {
				...
				if (cacheSpatialIndex && programAfterUnfolding.getRules().size() == 1) {
					...
					for (Function atom : programAfterUnfolding.getRules().get(0).getBody()) {
						...
						if (atom.getFunctionSymbol().equals(OBDAVocabulary.SFINTERSECTS)
								|| atom.getFunctionSymbol().equals(OBDAVocabulary.SFWITHIN)) {
	...
	private DatalogProgram splitSpatialJoin(DatalogProgram program) {
		...
		for (int k = 0; k < result.getRules().size(); k++) {
			...
			/*
			 * for (int i = 0; i < initial.getBody().size(); i++) { Set<Variable> varsNext =
			 * new HashSet<>(); TermUtils.addReferencedVariablesTo(varsNext,
			 * initial.getBody().get(i)); Set<Function> atomsNext = new HashSet<>();
			 * atomsNext.add(initial.getBody().get(i)); Set<Term> converted = new
			 * HashSet<Term>(); converted.addAll(varsNext); VarsToAtoms v2aNext = new
			 * VarsToAtoms(converted, atomsNext); v2aList.add(0, v2aNext); for (int j = 1; j
			 * < v2aList.size(); j++) { if (v2aNext.mergeCommonVar(v2aList.get(j))) {
			 * v2aList.remove(j); j--; } }
			 * 
			 * }
			 */
				...
				if (v2a.getVars().containsAll(varsInSpatial)) {
					...
					for (Variable spatialVar : varsInSpatial) {
						if (v2a.getVars().contains(spatialVar)) {
							...
							// result.addFirstRuleRule(createTempTable(v2a, initial, existentialVars,
							// varsInSpatial));
							// k++;
	...
	private CQIE createTempTable(VarsToAtoms cc, CQIE initial, Set<Variable> existentialVars,
			Set<Variable> varsInSpatial) {
		int id = Util.createUniqueId();
		DatabaseRelationDefinition replacement = questInstance.getMetaData().createDatabaseRelation(questInstance
				.getMetaData().getQuotedIDFactory().createRelationID("", OBDAVocabulary.TEMP_VIEW_QUERY + id));
		...
		for (Term t : cc.getVars()) {
			...
			if (existentialVars.contains(t) || varsInSpatial.contains(t)) {
				...
				if (varsInSpatial.contains(t) || isGeometry(t.toString(), initial)
						|| isGeometry(t.toString(), cc.getAtoms())) {
					// geometry
		...
		for (Function atom : initial.getBody()) {
			...
			if (StrabonParameters.isSpatialFunction(atom)) {
				...
				if (varsInSpatial.size() != 2) {
					varsInSpatial.clear();
					log.debug(
							"Spatial function with " + varsInSpatial.size() + " variables. Cannot split spatial join");
					continue;
					// return false;
				} else {
